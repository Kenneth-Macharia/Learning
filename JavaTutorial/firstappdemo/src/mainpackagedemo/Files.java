...
class Files {
    ...
    static String getFileInputs () {
        Scanner dataReader = new Scanner(System.in);
        String fileDetail = dataReader.nextLine();
        
        if (fileDetail == "") {
            dataReader.close();
            System.out.println("Field detail can't be empty!");

        } else if (fileDetail == "exit") {
            dataReader.close();
            System.out.println("Reading input cancelled.");
